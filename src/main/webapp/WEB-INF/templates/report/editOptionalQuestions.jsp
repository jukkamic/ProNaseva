...
<!-- MultipleChoiceQUestion -->
		...
		<c:forEach var="option" items="${question.options}">
			<label class="checkbox">											
				<sf:checkbox value="${option.option}" 
					path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].chosenSelections" label="${option.option}" />
			</label>
			<br>
		</c:forEach>
	</c:when>
	<c:otherwise>
			...
			<div class="btn-group" data-toggle="buttons">
				<c:forEach var="option" items="${question.options}" varStatus="optionsCounter">
					...
					<!-- Jos kysymykselle on ennalta tehty valinta esim. muokattaessa 
						raporttia uudelleen, kyseinen valintanappi näkyy valittuna. -->
					<c:choose>
					<c:when test="${optionalQuestionsAnswer.optionalAnswers[counter.index].chosenOptionIndex == optionsCounter.index}"> 
						<label class="btn btn-primary active">
						</c:when>
					<c:otherwise>
						<label class="btn btn-primary">
					...
					<c:choose>
						<c:when test="${option.radiobuttonText != null }">
							<sf:radiobutton id="button" path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].chosenOptionIndex" 
				 ...
				 <c:choose>
						<c:when test="${optionalQuestionsAnswer.optionalAnswers[counter.index].chosenOptionIndex == -1}"> 
							<label class="btn btn-default active">
							</c:when>
						<c:otherwise>
							<label class="btn btn-default">
		...
		<h4>Huomioita:</h4>
		<sf:textarea rows="5" style="width:100%;" path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].remarks" 
			value="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].remarks}" />
		<br><br>
	</c:if> 
	...
	<c:if test='${question["class"] == "class fi.testcenter.domain.question.TextQuestion"}'>
		...
		<c:choose>
			...
			<c:otherwise>
				<sf:input class="form-control" path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].answer" />
			</c:otherwise>
		</c:choose>
		<br>
	...
	<c:if test='${question["class"] == "class fi.testcenter.domain.question.CostListingQuestion"}'>
		<h3>${optionalQuestionCounter}. ${question.question}</h3>
			<c:forEach var="listQuestion" items="${question.questionItems}" varStatus="costListingAnswerCounter">
				<h4>${listQuestion}</h4>
				<sf:input style="width: 5em" path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].answers[${costListingAnswerCounter.index}]" /> €
				<br>
			</c:forEach>
			<br>
		<h4><b>${question.total}</b></h4>
		<sf:input style="width: 5em" path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].total" /> €
		<br><br>
	</c:if>
			...
			<div style="border-bottom: 3px solid #eee;">
				...
				<div class="btn-group" data-toggle="buttons">
					...
					<c:forEach begin="1" end="${question.numberOfItemsToChoose}" varStatus="importanceNumber">
						<c:choose>
							<c:when test="${optionalQuestionsAnswer.optionalAnswers[counter.index].answerItems[questionItemCounter.index].importance == importanceNumber.index}"> 
								<label class="btn btn-primary active">
								</c:when>
							<c:otherwise>
								<label class="btn btn-primary">
						...
						<sf:radiobutton id="button" path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].answerItems[${questionItemCounter.index}].importance"
								value="${importanceNumber.index}" /> &nbsp&nbsp${importanceNumber.index}&nbsp&nbsp 
								</label>
					</c:forEach>
					<c:choose>
							<c:when test="${optionalQuestionsAnswer.optionalAnswers[counter.index].answerItems[questionItemCounter.index].importance == -1}"> 
								<label class="btn btn-default active">
								</c:when>
							<c:otherwise>
								<label class="btn btn-default">
				...
				<div class="btn-group" data-toggle="buttons">
					...
					<c:forEach begin="1" end="${question.maxScoreForQuestionItem}" varStatus="score">
						<c:choose>
							<c:when test="${optionalQuestionsAnswer.optionalAnswers[counter.index].answerItems[questionItemCounter.index].score == score.index}"> 
								<label class="btn btn-primary active">
								</c:when>
							<c:otherwise>
								<label class="btn btn-primary">
						...
						<sf:radiobutton id="button" path="reportParts[${editReportPartNumber}].reportQuestionGroups[${questionGroupCounter.index}].answers[${questionCounter.index}].optionalAnswers[${counter.index}].answerItems[${questionItemCounter.index}].score"
								value="${score.index}" /> &nbsp&nbsp${score.index}&nbsp&nbsp 
								</label>
					</c:forEach>
						<c:choose>
							<c:when test="${optionalQuestionsAnswer.optionalAnswers[counter.index].answerItems[questionItemCounter.index].score == -1}"> 
								<label class="btn btn-default active">
								</c:when>
							<c:otherwise>
								<label class="btn btn-default">
