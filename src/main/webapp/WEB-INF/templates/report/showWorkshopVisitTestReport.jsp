...
<sf:form id="showReportForm" modelAttribute="readyReport" action="saveReadyReport" method="post"> 
						...
						<h4 class="panel-title">
							...
							<c:if test="${questionGroup.approvalRemarks == 'true' and (report.reportStatus != 'AWAIT_APPROVAL' or loginRole == '[ROLE_ADMIN]')}">
								<a
									...
									href="#${bootstrapPanelCounter}">${questionGroup.reportTemplateQuestionGroup.title} <span class="glyphicon glyphicon-warning-sign" style="color: red;"></span>
								</a>
							</c:if>
	...
	<!-- Answers loop -->
							...
							<c:if test="${loginRole == '[ROLE_ADMIN]'}">
								<br>
									...
									<sf:input id="approvalRemarks" path="reportParts[${reportPartCounter.index}].reportQuestionGroups[${questionGroupCounter.index}].answers[${answerCounter.index}].approvalRemarks" />
