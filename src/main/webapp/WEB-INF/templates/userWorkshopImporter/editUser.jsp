	...
	<div id="wrap">
		<div class="container">
			...
			<sf:form id="editUserForm" modelAttribute="user" method="post">
				...
				<label for="template">Käyttäjä edustaa maahantuojaa (asiakas-käyttäjä): </label>
				<sf:select style="width: auto; max-width: 100%" id="role" path="chosenImporterId"
					class="form-control">
					<option value=''>-- Valitse --</option>
					
					<c:forEach var="importer" items="${importerList}">
					${importer.name}
					
						<c:if test="${user.importer.id == importer.id}">
							
								<option value="${importer.id}" selected="selected">${importer.name}</option>
						</c:if>
						<c:if test="${user.importer.id != importer.id}">
				...
				<label for="workshops">Käyttäjän korjaamolista (asiakas-käyttäjä):</label>
				<br>
